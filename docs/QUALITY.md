...
6. **Module doc present** in every `.gd`-bearing directory of a policed layer — `MODULE.md` or `README.md`, since the tree practices both conventions (`check_module_docs.py`; `data/` exempt as generated catalog shards, ADR 0004). **Audited status: presence now enforced** (D0607) — the audit found `view/{audio,fx,hud,visuals}` undocumented; they carry `MODULE.md` now. "Current" remains uninstrumented: no cheap check verifies content freshness, and the gate claims only presence plus the existing 100-line cap on found `MODULE.md` files (`check_size_limits.py`).
...
10. **No softlock (scoped).** From the deepest authored point, scripted ascent reaches the surface. `test_softlock_ascent.gd` proves two shapes: a wide self-dug shaft in solid rock escaped by the mine-a-notch-and-mantle policy (~2,583 ticks), and the tutorial supply room's floor escaped by climbing alone (~3,045 ticks). **Audited status: now enforced, scoped** — the universal "from any reachable state" claim remains unprovable and unclaimed (D0602, D0606). Mutation-tested: disabling mining's break fails the shaft case while the walkable adit still passes.
...
13. **Schema.** Every file in `data/` validates — and `scenarios/` likewise, since D0620: it is scanned as a flat kind of its own, SCHEMA.yaml beside the records.
