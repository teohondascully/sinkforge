...
| 2. Architecture | **Closed** | `world_renderer.gd` 4601 -> 3557 lines across three extractions (3656 lines as of `096974e`, 2026-08-25 — the 3557 is what the extractions achieved, not a current size, and this row is in the present-tense table so it says which is which; re-read and corrected from a stale "3569 today" during an A+ upkeep pass), each cut against a measured ranking and each proven equivalent before it landed. Every candidate still in the file is rejected with its numbers rather than with a plan to get to it. `main.gd` and `factory_sim.gd` were measured and have no separable seam: their coupling is semantic, not a god-file boundary. Closed because the measurement says there is nothing left worth cutting, not because the time ran out — growth since the extractions is new content, not an unextracted seam, and reopening this area would need a fresh seam measurement showing otherwise, not just a bigger number. |
