...
> §1 scoped this as REWRITE (keep the 6-layer eval portfolio / actor-validity taxonomy / assistance
> ladder / judging protocol, cut the Freight-Winch-specific opening-loop content) — the "Edited
> 2026-08-25" header below suggests that edit was done, but it was never committed or added to
> `docs/README.md`'s normative table. Moved here rather than promoted to the live tree: the doc set that
> actually shipped is CONTEXT.md's current, leaner one, a later real decision this move doesn't
> override. Kept for provenance.
...
**Status:** specification and backlog only. This is not a harness layer, a release gate, or authority on
fun. It operationalises the first 20 minutes of the existing experience-evaluation program in
[`DIRECTOR_BRIEF.md`](DIRECTOR_BRIEF.md#4-experience-integration-evaluations). An agent may implement it
