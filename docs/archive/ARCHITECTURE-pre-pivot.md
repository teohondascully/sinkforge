> **SUPERSEDED 2026-08-25.** This document accurately describes the pre-pivot codebase now
> frozen in `legacy/`. It is archived rather than edited in place specifically so an agent
> reading legacy code can still find the architecture it was built against. See the new
> `docs/ARCHITECTURE.md` for the current, normative layered-instrument architecture.
