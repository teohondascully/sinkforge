> **ARCHIVED 2026-08-29 (queue Part E4).** Full verbatim WORKING.md history through this date -- every CLOSED round back to D0032/D0033 (the acceptance-suite stage), plus the still-open threads WORKING.md itself carried forward at reset time. Moved here because WORKING.md's own header requires it stay under 150 lines and resets when a stage closes, with durable content moved out first. This is the durable move: nothing here was deleted, only relocated. The genuinely OPEN items as of the reset (D0139's uncommitted resolve_floor investigation, the persistent-world GDD reversal thread, and the other standing-open items) are restated compactly in the new WORKING.md, not lost by this move -- this file is the full detail behind that compact restatement.
...
**Appended 2026-08-29, queue #2's own wrap:** the "IN PROGRESS -- queue #2" and "CLOSED -- 5-hour queue"
sections above, moved here verbatim from the live `docs/WORKING.md` once queue #2 itself closed, per
this file's own role as the append-only home for anything the live file rotates out.

**Appended 2026-08-29, fix-queue's own wrap:** the "CLOSED -- 5-hour autonomous queue #3" and "CLOSED --
5-hour autonomous queue #2" sections below, moved here verbatim from the live `docs/WORKING.md` once the
Codex certification fix queue (R1-R6, D0179-D0185) closed, per this file's own role as the append-only
home for anything the live file rotates out.
...
queue text and detailed report: this same archive file's own earlier-appended section.
...
## ARCHIVED 2026-08-29 (Slice 0's wrap) — FIX QUEUE, Codex certification findings

R1-R6 landed, 9 commits. Nothing certified — Codex re-verifies R1-R4 specifically on return.
Evidence: `docs/DECISIONS_LEDGER.md` D0179-D0185.

- **R1** (D0179) — `gate_status.py`'s absent-CI-as-PASS sibling bug (3rd find on this tool) fixed; three
  cases proven explicit (success->PASS, skipped->SKIPPED, absent->UNKNOWN), mutation-tested. A 4th attack
  found no further PASS-promotion path, but a real mirror bug in `gate_status_ci.py` (unconcluded step
  read as confident FAIL) — fixed too.
- **R2** (D0180) — gate 8's closure proof rebuilt; the old "moving sim/ aside" claim was disproved by a
  naive full-removal (it breaks the shared test base too). Real isolating removal re-verified in a scratch
  clone of HEAD, because the dirty local tree contaminates it.
- **R3** (D0181) — 7 more stale `economy_check`/`anvil` references swept.
- **R4** (D0182) — `project.godot`'s stale "CI needs no engine" comment corrected.
- **R5** (D0183) — ValueNoise's "one place" framing corrected to 4 enumerated float sites, 6 docs fixed.
- **R6** (D0184) — `grounded_no_floor` relabeled (cumulative trajectory, not independent-trial rate).
- **D0185** — the queue's own work drifted `docs/CORRECTIONS.md`'s freshness gate; reconciled.
- **D0186** — first real human `--play` sessions committed; `claims/C004` still BLOCKED (0 qualifying
  reveals across 4 sessions); `reveal_test_sparse` reproducibly throws `body ... left the world` on
  replay at the same position across two independent sessions. NOT diagnosed — collision-adjacent.
  The agreed next step (flag-gated diagnostic capture + replay-fidelity checker) is still NOT started;
  the legacy-revival migration took priority on the director's instruction.
