...
**Known, standing exception to the rule above: multiple float sites on the terrain-generation and RNG state path, not fixed-point** — a real gap between this section's own stated rule and what's actually shipped, diagnosed and tracked, not fixed. Corrected 2026-08-29 (fix queue R5): this section previously named only `sim/terrain_gen/value_noise.gd`'s cave-carving noise as "the" exception — an undercount. `docs/DECISIONS_LEDGER.md` D0183 enumerates all four known sites (`value_noise.gd`'s own hash-to-float conversion, two separate float computations inside `shaft_generator.gd`, and `core/split_rng.gd`'s `next_float()`/`next_range()`, used throughout terrain placement); D0171/D0172 remain the canonical crack reference and fix diagnosis. Determinism is proven WITHIN a single platform/build (same seed, two independent processes, bit-identical); it is NOT yet proven bit-identical ACROSS platforms (macOS vs. Linux) for anything that touches generated terrain, because IEEE 754 float arithmetic doesn't guarantee that across all these sites. The fix (converting the affected paths to `Fx`) is a real design cycle — it changes generated terrain for every existing seed — scoped in D0172, not yet decided.
