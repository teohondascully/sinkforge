...
##
## LIFTED from `legacy/scenes/particles.gd` (`docs/DECISIONS_LEDGER.md` D0216), unchanged. Nothing needed
## substituting: it has no dependency on any legacy type, takes world-space pixels and colours, and draws
## onto whatever `CanvasItem` it is handed. That is rarer in this batch than it sounds -- of the 21 code
## files in the migration map's LIFT set, six reference no missing type, and most of those six have no
## consumer in this build yet. This one does, which is why it came over now: `sim/mining` already reports
## `broke_this_tick`, `broke_cells` and `breach_this_tick` every tick, so the chips and the draught have
## real events to hang on rather than being a layer waiting for a game.
##
## `randf()` IS SAFE HERE and the header above says why, but it is worth being precise about the rule
## rather than trusting the sentence: `tools/layer_lint/no_engine_imports.py` polices `core/` and `sim/`
## only (`POLICED_DIRS`), and this is `view/`. The reason that scoping is correct, not lax, is that
## nothing in this file is ever read back by the sim -- a particle has no effect on any future tick, so
## its randomness cannot make a replay diverge. `tests/test_particles.gd` asserts exactly that boundary.
