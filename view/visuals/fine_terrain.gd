...
static func walked_surface(row: int) -> int:
	return row if row <= SURFACE_ROW_MAX else NO_SURFACE

## Legacy `HeightmapWorldGen.SURFACE_ROW_MAX` = `FLAT_SURFACE_ROW + 11`, row 31 — "the lowest valley
## floor", eleven rows of relief below its flat surface, in its 1-metre cells. This build's cell is a
## quarter-metre (`TERRAIN_CELLS_PER_METER` 4), so the same eleven metres of relief is 44 cells, measured
## down from this build's own surface row rather than from legacy's. Derived, not transcribed: the WG-4
## regime rule (D0305), and the reason it is not simply 31.
const SYNC_BAND: int = 1  ## legacy `src/core/fine_terrain.gd:26`
const SURFACE_RELIEF_METRES: int = 11
const SURFACE_ROW_MAX: int = ShaftGenerator.SKY_ROWS \
	+ SURFACE_RELIEF_METRES * ShaftGenerator.TERRAIN_CELLS_PER_METER
...
func rebake_region(cmin: Vector2i, cmax: Vector2i, solid_at: Callable, fine_solid_at: Callable,
	...
	# Legacy `src/core/fine_terrain.gd:26` `const SYNC_BAND: int = 1`, copied with its own reason: the
	# region bake must include one cell of pre-dig solidity around the dirty box or a stale bit smears
	# ~16 texels of wrong AO and moss, and the region bake stops being byte-identical to a full one.
	var band: int = SYNC_BAND
