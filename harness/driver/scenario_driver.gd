...
static func run(record: Dictionary) -> Dictionary:
	...
	if not WorldSeeder.stamp(world, items, machines, start_id, site_id):
		out["reason"] = "start '%s' did not stamp on site '%s'" % [start_id, site_id]
		return out
	...
	var env := Interface.Envelope.oracle_over(world.grid)
	...
	match StringName(record.get("agent", "")):
		...
		_:
			...
			return out
	bot.attach(world, items, body, iface, env)
	var rep: Dictionary = bot.execute(anchor)
	...
	while bot.ticks < budget:
		...
		for ev: Dictionary in o.events:
			if ev.get("kind") == want_kind and (want_id == &"" or ev.get("id") == want_id):
				...
				out["ok"] = true
				out["reason"] = "goal met"
				out["conservation_error"] = Invariants.check_item_conservation(items, 5)
				return out
		bot.tick(0)
