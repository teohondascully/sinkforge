...
the god-objects are decomposed along that seam: the simulation delegates its
heavy per-tick systems to focused, stateless modules (`water_flow`,
`power_flow`, `flora`, `fine_terrain`), and the renderer delegates its biggest
draw domains to painters (`sky_painter`, `terrain_painter`), each a pure move
that the harness verified behaviour-for-behaviour.
