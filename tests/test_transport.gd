...
func _test_flow_chains_a_hopper_into_a_forge_and_the_ingots_on_to_the_floor() -> void:
	...
	_steps(1)
	_check(int(forge.input_buffer[&"ore"]) == 2 and _status(Vector2i(5, 5)) == &"no_input", "two ore and no coal: the forge waits for fuel (D0483)")
	_feed(Vector2i(5, 5), &"coal", 1)
	_check(_status(Vector2i(5, 5)) == &"working", "...and with a coal it is working")
	_steps(2)
	_check(int(hop.input_buffer[&"ore"]) == 4 and _status(Vector2i(5, 2)) == &"blocked", "feed_cap 3 reached in the forge (two ore and the coal together): the hopper holds its other 4 (backed up) (%d)" % int(hop.input_buffer.get(&"ore", 0)))
	_steps(60)
	_feed(Vector2i(5, 5), &"coal", 2)                                     # fuel for the two crafts still to come
	_steps(200)
