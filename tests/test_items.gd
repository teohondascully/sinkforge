...
func _test_take_lode_refuses_a_full_pack_and_retires_a_dry_vein() -> void:
	...
	_check(items.take_lode(face) == &"ore_iron" and items.pack.count(&"ore_iron") == 1 and int(items.total_produced[&"ore_iron"]) == 1, "one unit taken, banked and ledgered")
	_check(items.world.deposits.lode_permille(face) == 500, "half the vein remains (500 per mille)")
	_check(items.take_lode(face) == &"ore_iron" and not items.world.deposits.lode.has(face) and items.world.deposits.lode_permille(face) == 0, "the second unit works the vein dry and it is retired")
