...
## needs no constant of its own (D0600). If the record moves, this test moves with it.
func _band() -> int:
...
## THE DIRECTOR'S REPORT, POSED (D0600): *"trees randomly spawn on flat platforms over shafts"*. A
...
## same span. Before D0600 only `col` was, and the shipped seed planted a tree with one half of its trunk
...
## ITEM 53, THE LOLLIPOP (D0601). Six crowns of three overlapping lobes replace the one ellipse every tree
