...
## The WORKS tab, bound like the other two.
var _works: BazaarWorks = BazaarWorks.new()
...
func _bazaar_wanted_h() -> float:
	...
	match bazaar_tab:
		TAB_WORKS:
			need = float(_works_page()._works_rows_needed()) * BAZAAR_ROW_H
		TAB_BENCH:
...
## Hand the WORKS tab the surface, the focused row, and the stock it lists.
func _works_page() -> BazaarWorks:
	_works._canvas = _canvas
	_works._font = _font
	_works._sim = _sim
	_works._icons = _icons
	_works.probing = probing
	_works.panel_probe = panel_probe
	_works.bazaar_row = bazaar_row
	_works.cat = _catalogue()
	return _works


## Forwarded for the shell, as the other tabs are. `works_columns` is also read by a layout check, and
## `works_window_first` is pure arithmetic the Hud exposes statically, so both keep their names here.
func _tab_works(g: Dictionary) -> void:
	_works_page()._tab_works(g)


func works_columns(rows: int) -> Dictionary:
	return _works_page().works_columns(rows)


static func works_window_first(count: int, capacity: int, base: int, cursor: int) -> int:
	return BazaarWorks.works_window_first(count, capacity, base, cursor)
