...
## The tech tree (docs/PROGRESSION.md 5). Each tech consumes one sample of a signature material, which
## binds research to descent, plus a volume of refined goods, and unlocks crafting its machines. Plain
## static data read by the craft gate, the controller's verb, the HUD rows and the headless tests.
## `automation` is tutorial-priced; `power` is the first real wall at 12 ingots.
##
...
const TECHS: Dictionary = {
	&"automation": {
		"name": "Automation",
		"requires": &"",                       # no prereq and the first rung
	...
	&"prospecting": {
		"name": "Prospecting",
		"requires": &"automation",             # the tree's first branch at the same tier as Power
		"sample": &"ore",                      # analyze a vein and learn what its echo sounds like
		"cost": {&"ingot": 4},
		"unlocks": [&"scanner"],               # the handheld sonar that finds the veins
		                                       # that pay the 12-ingot Power wall right after
	},
	&"crosscutting": {
		"name": "Crosscutting",
	...
	&"ironworks": {
		"name": "Ironworks",
		"requires": &"descent",
		"sample": &"iron",                     # analyze L2's signature ore that you must breach to hold
		"cost": {&"ingot": 10},
	...
	&"machining": {
		...
		"unlocks": [&"plate_press", &"gear_mill", &"h_drill"],   # the crafter modules + the Borer
		                                       # (the Borer is machining's payoff, priced in plates and gears)
	},
	&"galleries": {
		"name": "Galleries",
		"requires": &"machining",
		...
		"unlocks": [&"drift_rig"],             # the powered gallery machine that sorts pay from spoil at
		                                       # the face (docs/DRIFT.md). Deliberately after Power and the
		                                       # Borer: it answers a problem you have personally hauled.
	},
	&"packing": {
		"name": "Packing",
		"requires": &"galleries",
		"sample": &"stone",                   # analyze the spoil again, asking what it is for: the Crusher,
		                                       # gravel and a gallery that holds water (docs/DRIFT.md §4)
		"cost": {&"plate": 2, &"gear": 2},
		"unlocks": [&"crusher"],
	},
	&"enrichment": {
		"name": "Enrichment",
		"requires": &"machining",
		"sample": &"rich_ore",                 # analyze the sparkling rock you spotted on the shelf
		"cost": {&"iron_ingot": 4},
		"unlocks": [&"blast_furnace"],         # two ingots from one rich ore: the quality axis pays out
	},
