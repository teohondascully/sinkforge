...
## Picks that differ in shape rather than in speed (`docs/BITS.md` 2-3). `MiningRules.TOOLS` used to gate
## which rock a tier may bite and to multiply break speed 1.0 / 1.7 / 2.6. The speed axis is deleted, leaving
## the drive (a pick's tier, gating what may be bitten) and the bits below: interchangeable cutting heads
## deciding what one swing takes. All are collectable and none is ever removed.
##
...
## `cap`: how far a blow may calve along a seam (see `Seams`); 1 means it does not follow the grain. `keeps`
## false is the Broad's pulverise. `grain_only` is the Wedge: it splits rock or does nothing. `recovery` is
## seconds added before the next charge may begin. `ray` marks a driven line rather than a block: it stops at
## the first non-solid cell, so a lance cannot take rock beyond a chamber.
