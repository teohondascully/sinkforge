...
## THE FLOW STEP — run every tick (see tick()). Deterministic + integer + snapshot-based, so it is
...
func _flow_water() -> void:
	...
	var snap: Dictionary = water.duplicate()
	# Deterministic scan order over the wet cells (top-to-bottom, then left-to-right).
	var wet: Array[Vector2i] = []
	for cv: Variant in snap:
		wet.append(cv)
	wet.sort_custom(_water_cell_less)
	for c: Vector2i in wet:
		var level: int = int(water.get(c, 0))                 # read LIVE (a higher cell may have topped it up)
		if level <= 0:
			continue
		var below: Vector2i = c + Vector2i(0, 1)
		if not in_bounds(below) or solid.has(below):
			continue
		var room: int = WATER_MAX - int(water.get(below, 0))
		if room <= 0:
			continue
		var moved: int = mini(level, room)
		water[below] = int(water.get(below, 0)) + moved
	...
	for c: Vector2i in wet:
		...
		var lo: int = c.x
		while _water_open(Vector2i(lo - 1, c.y)):
			lo -= 1
		var hi: int = c.x
		while _water_open(Vector2i(hi + 1, c.y)):
			hi += 1
		...
		var cap_total: int = span * WATER_MAX
		if sum > cap_total:
			for x3: int in range(lo, hi + 1):
				water[Vector2i(x3, c.y)] = WATER_MAX
			water[Vector2i(lo, c.y)] = WATER_MAX + (sum - cap_total)   # overflow rides the left cell; falls next tick
			continue
...
func _water_open(cell: Vector2i) -> bool:
	return in_bounds(cell) and not solid.has(cell)


## Deterministic cell ordering for the water sweeps: top-to-bottom (row), then left-to-right (col).
func _water_cell_less(a: Vector2i, b: Vector2i) -> bool:
	if a.y != b.y:
		return a.y < b.y
	return a.x < b.x
