...
add "check_rules_registry"            "res://tools/check_rules_registry.gd"
...
add "check_carry_cap (the pack fills)" "res://tools/check_carry_cap.gd"
add "check_hopper_objective (routes coal)" "res://tools/check_hopper_objective.gd"
add "check_hopper_status (backed-up state)" "res://tools/check_hopper_status.gd"
add "check_pile_reach (sealed by your own line)" "res://tools/check_pile_reach.gd"
add "check_seal_desire (the wall names the reward)" "res://tools/check_seal_desire.gd"
...
if [ "${SF_NO_LOCK:-0}" != "1" ]; then
	...
	while true; do
		...
		if [ -n "$holder" ] && ! kill -0 "$holder" 2>/dev/null; then
			if lock_steal; then
				echo "  (cleared a stale harness lock: pid $holder was gone)"
			fi
			continue
		fi
		# A lock directory that has named no owner for ten seconds is debris, not a run.
		if [ -z "$holder" ] && [ "$waited" -ge 10 ]; then
			if lock_steal unowned; then
				echo "  (cleared a harness lock that had named no owner)"
			fi
			continue
		fi
