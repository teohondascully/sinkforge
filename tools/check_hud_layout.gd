...
##
## COMPREHENSIVE_AUDIT Â§413 asks for exactly this and nothing in the suite provides it: a scripted matrix
## of HUD states â title, arrival, hint, alert, hover, pause, Bazaar â with zero bounding-box overlap and
...
##
##   top-left     the depth readout AND the "â¶â¶ Nx" fast-forward chip
##   top-right    the FORGED counter AND the machine inspector, which is documented as sitting "under"
##                it â a relationship maintained by two independent constants
...
##
## HOW IT OBSERVES. The HUD is immediate-mode â no Control nodes, nothing to read off the scene tree â so
...
##   OVERLAP, only unmodal.      A Bazaar/help/settings overlay is SUPPOSED to sit over the furniture â
...
func _run() -> void:
	...
	# is not judged there â see the header. Everything is judged for clipping.
	var states: Array[Dictionary] = [
		{"name": "the bare screen", "modal": false, "set": {}, "keep": "bare"},
		{"name": "paused", "modal": false, "set": {"_paused": true}},
		{"name": "running fast (the â¶â¶ chip beside the depth readout)", "modal": false,
		...
		# allowed to cover furniture, and the big map's rule is the opposite â the furniture stands down
		...
		# large form spans x 181..459 / y 41..319 and the arrival plate sits centred at y ~62..112 â so
		...
		# column â so the inspector never collides"; that is false for a 128-wide world, and this row is
		...
		# already been moved once for exactly this reason â it used to print across the objective line â
	...
	for st: Dictionary in states:
		...
		_check(off.is_empty(), "%s: all %d panels are on screen%s"
			% [name, rects.size(), "" if off.is_empty() else " â OFF: " + ", ".join(off)])

		# FOOTPRINT, reported and not judged here â see `_report_footprint`. IT GOES ABOVE THE MODAL
		...
		_check(hits.is_empty(), "%s: no two panels collide%s"
			% [name, "" if hits.is_empty() else " â " + "; ".join(hits)])
	...
	# to differ from each other too â if they all reported the same panels, the matrix would be one state
	# tested eleven times.
	_check_help_text()
	...
	# the stated reason for the panel-count floor â but a count cannot tell one screen from eleven copies of
	...
	# same screen by design â the big map alone, with a machine hovered, and with a stratum arrival held â
	...
	_check(twins.is_empty(), "every state drew a different screen from every other%s"
		% ["" if twins.is_empty() else " â IDENTICAL: " + "; ".join(twins)])
...
## THE BIG MAP IS THE SCREEN, so nothing may be left under it â and nothing may be left HALF under it.
##
...
## standing it down at `hud.gd:699`, and the second â a 46x44 panel at (297,295), overlap 46x24 â was left
...
## `hud.gd:263`. It never moved. The matrix asked for `_minimap_mode` **1**, which is the CORNER map â
## 122x122 in the top-right, 142px clear of the bar â so the state that collides was never entered.
##
...
## HAPPENED to draw, so it also passes the moment a panel stops being drawn at all â and `_draw_minimap`
...
func _check_big_map(bare: Array[Rect2], big: Array[Rect2]) -> void:
	...
	_check(buried.is_empty(), "the big map has nothing buried under it%s"
		% ["" if buried.is_empty() else " â " + "; ".join(buried)])
	...
	_check(bar.size.y >= MIN_PANEL and bar.position.y > Hud.CANVAS.y * 0.5,
		"the bare screen drew the pack bar along the bottom (%s) â there was something to stand down" % bar)
	...
	#    deleting it would have been invisible. Proved by DIFFERENCE in both directions â absence alone
	...
	_check(plate.size.y >= MIN_PANEL,
		"the bare screen drew the goal plate at top-centre (%s) â there was something to suppress" % plate)
	var survivor: Rect2 = _goal_plate(big)
	_check(survivor.size.y < MIN_PANEL,
		"the big map suppresses the goal plate (found %s)" % survivor)


## THE MACHINE INSPECTOR â the panel this matrix has claimed to judge since it was written, and never has.
##
...
## and this proves the warp WORKED â because a hover fixture that silently fails to hover is the same
...
func _check_hover(bare: Array[Rect2], hover: Array[Rect2], big_hover: Array[Rect2]) -> void:
	...
	# did not take and every verdict below is void â reported as a FIXTURE failure, not as a HUD verdict.
	_check(hover.size() > bare.size(),
		"hovering a machine drew MORE panels than the bare screen (%d vs %d) â the cursor reached it"
	...
	# THE COLLISION. hud.gd:837 claims the large map "is centred, off this column â so the inspector never
	...
	_check(over_map.is_empty(), "the inspector stands down under the big map%s"
		% ["" if over_map.is_empty() else " â " + "; ".join(over_map)])
...
## (`:602`) and PAUSED at y=50 (`:283`) â so the anchor identifies it and nothing else does. Deliberately
...
func _snapshot(hud: Hud, st: Dictionary) -> Array[Rect2]:
	...
	# is overwritten before the frame draws â the first version of this layer did exactly that and every
	...
	# mouse on every `_process` â unconditionally, pause included (main.gd:709). So the assignment was
	...
	# wherever the operator's mouse happened to be sitting â so those rows did not merely test nothing,
	...
	# and every panel was recorded two or three times â which the overlap test dutifully reported as each
	...
	# screen silently gained a sixth panel. That is the same defect as the uncontrolled mouse â a row
	# whose content depends on something the fixture does not own â and it surfaced the moment
	...
	# Warping before the settle frames let the body drift under them â at `_time_scale_idx: 3` the sim
	# advances further per frame â so the camera moved after the cursor was placed and the aim no longer
...
## all of it â and the settings page has been drawing the last two of its twenty-two key bindings below
...
## through. Judging THOSE cannot drift from what is clickable, because they are what is clickable â the
...
##
## This floor used to be a single 26 â "sliders, chips, the mute, the reset, and one per binding" â over a
...
## still all on screen, still all on the plate â that was the defect this assertion was written for and it
...
## frames elapsed before the shutter, not of the layout â this project has paid for that lesson twice.
...
func _check_controls_reachable() -> void:
	...
	for cat: int in SETTINGS_CAT_MIN.size():
		...
		_check(off.is_empty(), "settings %s: all %d clickable controls are on screen%s"
			% [name, hits.size(), "" if off.is_empty() else " â OFF: " + ", ".join(off)])
		...
		_check(spilled.is_empty(),
			"...and every one sits on the %.0fx%.0f plate that owns them%s"
				% [plate.size.x, plate.size.y, "" if spilled.is_empty() else " â OFF THE PLATE: "
					+ ", ".join(spilled)])
		...
		# page was not â the control failing for the same reason as the subject, and saying nothing about
	...
	# NOT A FLOOR â a ledger. The three faces together must still offer at least what the one crowded page
...
## panel â which for a modal state is the modal's own plate.
...
## â `main.gd` asks for the change and never pushes the field. That distinction is not pedantry: this
...
## outside its own panel, over the darkened world behind it â and the panel it overflowed reported a
...
## size and the same width constant the drawing code uses â not a copy of them.
func _check_help_text() -> void:
	...
	_check(over.is_empty(), "every CONTROLS line fits its %.0fpx column (widest %.0fpx)%s"
		% [budget, widest, "" if over.is_empty() else " â OVER: " + "; ".join(over)])
	...
	_check(Hud.HELP_LINES.size() >= 12 and widest > budget * 0.5,
		"the card carries %d lines and the widest uses %.0f%% of its column â there was text to measure"
...
##
## `FactorySim.inventory_slots()` has NO CAP â one entry per item type, and the universe is twenty machine
...
## pack". Reachable on frame one of a dev start â the kit is ten types and the starter pickaxe is an
...
func _check_pack_window() -> void:
	...
	# The same pack overflowing. The SETUP is that the bar now shows fewer wells than the pack holds â that
...
## real ones â the bar looks up sprites and labels by id, and a made-up id would exercise a drawing path
## no player ever sees.
...
## GDScript â verified against 4.6.2 rather than assumed. So the guard fell open on every frame of every
...
## flag, on a deleted probe â so the flag is turned back ON afterwards and the probe has to FILL. Absence
## is only evidence once the instrument has proved it can detect presence.
## THE ANNOUNCE CHANNEL HAS ONE OCCUPANT AT A TIME â `P1`'s only structural rule, and the matrix above
...
## already open â so that row exercises the direction that was already guarded, gets a deferral, and is
...
## UI-06 â NO LESSON MAY GROW. A RATCHET, SET FROM MEASUREMENT AND NEVER FROM TASTE.
##
## The ticket asks for *"a capture-reviewed max height/coverage for non-modal lessons"* and guards it with
## *"do not treat panel area alone as an aesthetic score"* â so this caps the bubble and claims nothing
## about whether the bubble is good. It is a floor under a regression, not a verdict on a design.
##
## **61px is three drawn lines at size 11, and it is where the game already is** â measured across all
...
## near the cap reports 250.0 by construction and a `w <= 250` assertion could not fail for any string â
...
func _check_lesson_footprint() -> void:
	...
	_check(over.is_empty(), "every lesson fits the %.0fpx tutorial ceiling%s"
		% [LESSON_MAX_H, "" if over.is_empty() else " â OVER: " + ", ".join(over)])
...
## UI-07 â THE HELPER INVENTORY HAS TO BE TOTAL OR IT IS DECORATION.
##
...
func _check_helper_registry() -> void:
	...
	_check(untagged.is_empty(), "every drawing surface is classified by UI-07%s"
		% ("" if untagged.is_empty() else " â UNTAGGED: " + ", ".join(untagged)))
	...
	_check(stale.is_empty(), "...and the table names nothing that has been deleted%s"
		% ("" if stale.is_empty() else " â STALE: " + ", ".join(stale)))
	...
	# version of this used "nearly the whole centre column" as a generous bound â 600px wide â and failed a
	...
	_check(not chip.intersects(plate),
		"the PAUSED chip %s clears the arrival plate's widest possible core %s â two criticals, one strip"
			% [chip, plate])
...
func _check_probe_is_off() -> void:
	...
	# POSITIVE CONTROL â the same three frames with the flag on.
	Hud.probing = true
	Hud.panel_probe = ([] as Array[Rect2])
	Hud.hotbar_probe = {}
	for _i: int in 3:
	...
	_check(recorded > 0 and recorded_bar,
		"the probe records while it is on â %d panels, hotbar %s (if this fails the check below means nothing)"
			% [recorded, "yes" if recorded_bar else "no"])
	_check(leaked == 0 and not leaked_bar,
		"...and records NOTHING while it is off â %d panels, hotbar %s"
...
## the loop drew one per iteration unconditionally, so the number was `n` restated â decided by `clampi`
## before a pixel moved. The failure this is really guarding against is positional: derive each well's `sx`
## from the PACK index instead of the WINDOW slot â one character, and the same mistake the name plate
## actually shipped with â and the wells march off their own backing and off the canvas while every count
...
func _wells_report(probe: Dictionary) -> String:
	var wells: Array = probe.get("wells", []) as Array
	if wells.is_empty():
		return " â NO WELLS DRAWN"
	var first: Rect2 = wells[0] as Rect2
	var last: Rect2 = wells[wells.size() - 1] as Rect2
	return " â %d wells, x %.0f..%.0f, backing %s" % [wells.size(), first.position.x,
...
## `_panel()`, `_round_rect()` and the arrival scrim. It does not see bare `draw_rect`/`draw_string` â
...
##
## Union area, not summed area â overlapping panels must not be counted twice, and the modal states
...
##
## The floor below is not a guess about what a good HUD costs â this project has been wrong four times
...
##
## Only the bare screen is held. The modal states are supposed to cover the screen â that is what a modal
## is â and ratcheting them would be asserting that the Bazaar must not grow, which is not a thing anyone
## has decided.
##
## THE NUMBER: measured 7.83% on three consecutive runs, identical to the digit. The ceiling is 8.0%, and
## the 0.17pp of headroom is for ONE stated reason â the goal plate and the inspector size themselves to
...
func _report_footprint(names: Array[String], fracs: Array[float]) -> void:
	...
	print("    footprint = union area of HUD PANELS / canvas. Panels only â bare draw_rect/draw_string")
	...
	# the bare screen. That is a definitional property rather than a tuned one â no number to calibrate,
	...
	_check(quiet.is_empty(), "...and every modal covers more than the bare screen%s"
		% ["" if quiet.is_empty() else " â NOT: " + ", ".join(quiet)])
