...
func _judge() -> void:
	...
	var across: Array = _sweep(r, data, solid, fcols, frows, sub, SOIL_CLEAR, true)
	var down: Array = _sweep(r, data, solid, fcols, frows, sub, SOIL_CLEAR, false)

	var ok: bool = true
	ok = _report("across a face (left-right)", across) and ok
	ok = _report("down a face (top-bottom)", down) and ok
...
func _sweep(r: WorldRenderer, data: PackedByteArray, solid: PackedByteArray, fcols: int, frows: int,
		sub: int, from_row: int, horizontal: bool) -> Array:
	...
	for fy: int in range(from_row, frows - 1):
		for fx: int in range(1, fcols - 1):
			...
			if not _deep_interior(solid, fcols, frows, fx, fy) or not _deep_interior(solid, fcols, frows, nx, ny):
				continue
...
func _report(label: String, m: Array) -> bool:
	var seam: float = m[0]
	var body: float = m[1]
